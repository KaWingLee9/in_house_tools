...
While SOAPy used z-score as the statistic to reflect the proximity between two cell types, here we used the P value against the null hypothesis generated by permutation as statistic. The codes are simply modified from SOAPy, and the parameters are totally same as `sp.tl.neighborhood_analysis`. P values are reported while positve and negative represented interaction and avoidance, repsectively.  

```python
exec(open('./neighbourhood_pval.py').read())

neighborhood_analysis(adata,sample_key='image_id',method='include',
                      cluster_key='cell_type',n_iters=1000,n_jobs=100)
```

__Reference__: Wang, H.#, Li, J.#, Jing, S., Lin, P., Qiu, Y., Yan, X., Yuan, J., Tang, Z., Li, Y., Zhang, H., Chen, Y., Wang, Z., & Li, H.* (2025). SOAPy: a Python package to dissect spatial architecture, dynamics, and communication. Genome biology, 26(1), 80. https://doi.org/10.1186/s13059-025-03550-5   
...
relative_distance_analysis(adata,sample_key='image_id',cluster_key='cell_type',CT1='Macrophage',CT2='T cell',CT3='Tumor_cell',
...
## Citation
Wang, H.#, Li, J.#, Jing, S., Lin, P., Qiu, Y., Yan, X., Yuan, J., Tang, Z., Li, Y., Zhang, H., Chen, Y., Wang, Z., & Li, H.* (2025). SOAPy: a Python package to dissect spatial architecture, dynamics, and communication. Genome biology, 26(1), 80. https://doi.org/10.1186/s13059-025-03550-5
