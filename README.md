# In-house tools/pipelines by Kelvin Lee
For any questions, please contact with me. My contact information is available at:  
https://github.com/KaWingLee9

## Table of Contents
### Data analysis pipeline
+ [OncoMolPath: A comprehensive WXS and RNA-Seq workflow for tumor molecular pathology and precision medicine](https://github.com/KaWingLee9/OncoMolPath)  
+ [Data analysis of RNA-Seq](https://github.com/KaWingLee9/in_house_tools/tree/main/BulkRNASeq)  
+ [Data analysis pipeline of multiplexed imaging data (Python)](https://github.com/KaWingLee9/in_house_tools/tree/main/multiplexed_images_pipeline)  
+ [Tumor evolution analysis from multi-omics data]  
+ [Intra-tumoral and inter-tumoral heterogeneity dissected by scRNA-Seq]
### Small tricks and tools
+ [Custom R visualization functions](https://github.com/KaWingLee9/in_house_tools/tree/main/visulization)  
+ [Conver Seurat object (v4) to scanpy object (for both __single-cell__ and __spatial-omics__ data)](https://github.com/KaWingLee9/in_house_tools/tree/main/Seurat2Scanpy)
+ Clustering result assessment
+ [Meta analysis](https://github.com/KaWingLee9/in_house_tools/tree/main/meta_analysis)
+ [Curated gene sets](https://github.com/KaWingLee9/in_house_tools/tree/main/CuratedGeneSets)

